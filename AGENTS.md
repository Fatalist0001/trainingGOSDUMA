...
4. **Композиционность** — предсказываем доли ключевых партий независимыми регрессиями (по одной на партию). В датасете (region-level) только 3 партии: UR, KPRF, LDPR; в парламентских годах они покрывают ~78% голосов (остальное — прочие партии вне данных). Таргеты — реальные доли, **не нормализуются**. ⚠️ Нюанс реализации: `run_single_model_backtest` **пост-хок нормализует** предсказания плоских моделей к 100% (Approach A из PLAN §6), временные модели — нет. Запланирован пересчёт без нормализации.
